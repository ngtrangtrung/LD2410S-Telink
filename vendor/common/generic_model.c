...
#include "../mesh/user/default_network.h"
...
typeGen_handleCommandSigOnOffSet  pvGen_handleCommandSigOnOffSet = NULL;
...
{
	DBG_GEN_MODEL_SEND_STR("\n ...send_device_status_with_check_dst_adr");
	...
	if(cb_par->adr_dst < ADR_GROUP_START_POINT){
		...
		u16 binding_addr = get_group_binding_adr(cb_par->model_idx);
		if((binding_addr != ADR_UNASSIGNED)
				&& (binding_addr == cb_par->adr_dst)) {
			light_publish_status_delay(cb_par->model_idx, TIMER_1S + (rand()%TIMER_3S));
		}
		else {
			light_publish_status_delay(cb_par->model_idx, 5000 + (rand() % 30000));
		}
	}
	return err;
}

static void mesh_cmd_sig_g_onoff_manual_send_st(u8 *par, int par_len, mesh_cb_fun_par_t *cb_par)
{
	...
	if(par_len >= sizeof(mesh_cmd_g_onoff_set_t)){
		if(is_valid_transition_step(p_set->transit_t)){
			if(!is_transition_need(p_set->transit_t, p_set->delay)){
				send_device_status_with_check_dst_adr(cb_par);
			}
			else{
				mesh_cmd_g_level_st_t level_st;
				u8 present_onoff, target_onoff;
				light_g_level_get((u8 *)&level_st, cb_par->model_idx, ST_TRANS_LIGHTNESS);
				present_onoff = get_onoff_from_level(level_st.present_level);
				target_onoff = get_onoff_from_level(level_st.target_level);
				if(present_onoff == target_onoff){
					send_device_status_with_check_dst_adr(cb_par);
				}
			}
		}
...
void gen_callback_init(typeGen_handleCommandSigOnOffSet callbackFunc)
{
	if(callbackFunc != NULL){
		pvGen_handleCommandSigOnOffSet = callbackFunc;
	}
}
...
{
	DBG_GEN_MODEL_SEND_STR("***mesh_cmd_sig_g_onoff_set***");


	int err = 0;
	g_addr_dest_arr[cb_par->model_idx] = cb_par->adr_dst;
    ...
    if(pvGen_handleCommandSigOnOffSet != NULL){
    	pvGen_handleCommandSigOnOffSet(par, par_len, cb_par);
    }
	...
	if(cb_par->op_rsp != STATUS_NONE){
		...
		mesh_cmd_sig_g_onoff_manual_send_st(par, par_len, cb_par);
	}
...
int mesh_cmd_sig_g_on_powerup_set(u8 *par, int par_len, mesh_cb_fun_par_t *cb_par)
{
	...
	DBG_GEN_MODEL_SEND_STR("mesh_cmd_sig_g_on_powerup_set");
...
void mesh_handle_reset_node_if_cannot_receive_add_ak_msg(void){
	if(wait_add_ak.f_wait == TRUE){
		if(clock_time_exceed_ms(wait_add_ak.start_t_ms, WAIT_TIME_ADD_AK_DEFAULT)){
			setup_factory_reset_with_delay(TRUE, 1000);
...
int mesh_cmd_sig_cfg_appkey_set_user_handle(u8 *par, int par_len, mesh_cb_fun_par_t *cb_par)
{
	...
	// BLINK LED
	static u32 blink_led_add_ak_last_time = 0;
	if((blink_led_add_ak_last_time == 0)
				|| (clock_time_exceed_ms(blink_led_add_ak_last_time, 30000))){
	    rf_link_light_event_callback(LGT_CMD_SUC_ADD_APPKEY);
	    blink_led_add_ak_last_time = clock_time_ms();
	}
	// SEND SPECIAL CONFIG
	vd_setup_send_all_config_node_params_delay(SLOW_REPORT);
	return err;
}
...
const mesh_cmd_sig_func_t mesh_cmd_sig_func[] = {
	...
	{APPKEY_ADD, 0, SIG_MD_CFG_CLIENT, SIG_MD_CFG_SERVER, mesh_cmd_sig_cfg_appkey_set_user_handle, APPKEY_STATUS},
    ...
    {CFG_MODEL_SUB_ADD, 0, SIG_MD_CFG_CLIENT, SIG_MD_CFG_SERVER, mesh_cmd_sig_cfg_model_sub_user_set, CFG_MODEL_SUB_STATUS},
    {CFG_MODEL_SUB_DEL, 0, SIG_MD_CFG_CLIENT, SIG_MD_CFG_SERVER, mesh_cmd_sig_cfg_model_sub_user_set, CFG_MODEL_SUB_STATUS},
    {CFG_MODEL_SUB_OVER_WRITE, 0, SIG_MD_CFG_CLIENT, SIG_MD_CFG_SERVER, mesh_cmd_sig_cfg_model_sub_user_set, CFG_MODEL_SUB_STATUS},
    {CFG_MODEL_SUB_DEL_ALL, 0, SIG_MD_CFG_CLIENT, SIG_MD_CFG_SERVER, mesh_cmd_sig_cfg_model_sub_user_set, CFG_MODEL_SUB_STATUS},
...
const u16 control_light_op_arr[] = {G_ONOFF_SET, G_ONOFF_SET_NOACK, LIGHTNESS_SET, LIGHTNESS_SET_NOACK,
 #if LIGHT_TYPE_CT_EN
