...
static typeCommon_HandleTxMessageFunc pvCommon_HandleTxMessage = NULL;
...
void publish_status_periodically(void)
{
	for(u8 i = 0; i < LIGHT_CNT; i++){
	    light_publish_status_delay(i, rand()% 5000);
    }
}

void set_publish_status_periodically_interval(void){
	publish_status_periodically_ms =
...
int mesh_rc_data_layer_access_cb(u8 *params, int par_len, mesh_cb_fun_par_t *cb_par)
{
    ...
    if(!is_cfg_model(p_res->id, p_res->sig)){ // user should not handle config model op code
        ...
        if(IS_VENDOR_OP(cb_par->op)){
            if((cb_par->op >= VD_OP_RESERVE_FOR_TELINK_START) && (cb_par->op <= VD_OP_RESERVE_FOR_TELINK_END)){
            	...
            	DBG_MESH_COMMON_SEND_STR("\n mesh_rc_data_layer_access_cb ERROR");
    ...
    DBG_MESH_COMMON_SEND_STR("\n mesh_rc_data_layer_access_cb SUCCESS");
...
void handle_tx_message_callback_init(typeCommon_HandleTxMessageFunc pCallback){
	if(pCallback != NULL){
		pvCommon_HandleTxMessage = pCallback;
	}
}

int app_hci_cmd_from_usb_handle (u8 *buff, int n) // for both usb and uart
{
	if(pvCommon_HandleTxMessage != NULL){
		pvCommon_HandleTxMessage(buff, n);
	}
	return 0;
}
