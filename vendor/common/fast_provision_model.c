...
const u8 mac_header[] = {0x04, 0x95, 0xF1, 0x48 , 0x15 , 0xC7 , 0x53 , 0x7B , 0xC9 , 0x0F};
...
void mesh_revert_network()
{
	...
	if(provision_mag.gatt_mode == GATT_PROVISION_MODE){		
		...
		if((!fast_prov.not_need_prov)&&(mesh_fast_prov_sts_get() == FAST_PROV_COMPLETE)){
            ...
            // PUBLISH
        	foreach(i, ELE_CNT) {
        		light_publish_status_delay(i, 5000+(rand()%5000));
        	}
        	// SEND SPECIAL CONFIG
...
static uint8_t en_fast_revert_network = FALSE;
...
void mesh_fast_prov_proc()
{
	...
	if((mesh_fast_prov_sts_get() == FAST_PROV_RESET_NETWORK)
	...
	switch(fast_prov.rcv_op){
		...
		case VD_MESH_PROV_DATA_SET:
			if(fast_prov.cur_sts == FAST_PROV_SET_ADDR){				
                ...
                if(en_fast_revert_network == TRUE) {
...
{
#ifdef FAST_PROVISION_DBG_EN
	DBG_FAST_PROVISION_SEND_STR("\n ___cb_vd_mesh_set_addr: ");
	foreach(i, par_len) {
		DBG_FAST_PROVISION_SEND_STR(" ");
		DBG_FAST_PROVISION_SEND_BYTE(*(par+i));
	}
#endif
	int err = -1;
	if(fast_prov.not_need_prov){
...
{
#ifdef FAST_PROVISION_DBG_EN
	DBG_FAST_PROVISION_SEND_STR("\n ___cb_vd_mesh_set_provision_data: ");
	foreach(i, par_len) {
		DBG_FAST_PROVISION_SEND_STR(" ");
		DBG_FAST_PROVISION_SEND_BYTE(*(par+i));
	}
	...
	DBG_FAST_PROVISION_SEND_STR("\n par_len: ");
	DBG_FAST_PROVISION_SEND_INT(par_len);

	DBG_FAST_PROVISION_SEND_STR("\n MASTER_LEN: ");
	DBG_FAST_PROVISION_SEND_INT(sizeof(provison_net_info_str) + sizeof(mesh_appkey_set_t));
#endif
	if(fast_prov.not_need_prov){
		return -1;
	}
...
{
#ifdef FAST_PROVISION_DBG_EN
	DBG_FAST_PROVISION_SEND_STR("\n ___cb_vd_mesh_provision_confirm: ");
	foreach(i, par_len) {
		DBG_FAST_PROVISION_SEND_STR(" ");
		DBG_FAST_PROVISION_SEND_BYTE(*(par+i));
	}
...
{
#ifdef FAST_PROVISION_DBG_EN
	DBG_FAST_PROVISION_SEND_STR("\n ___cb_vd_mesh_provision_complete: ");
	foreach(i, par_len) {
		DBG_FAST_PROVISION_SEND_STR(" ");
		DBG_FAST_PROVISION_SEND_BYTE(*(par+i));
	}
