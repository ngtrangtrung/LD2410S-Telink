...
#define DBG_MESH_NODE_SEND_STR(x)   Dbg_sendString((s8*)x)
#define DBG_MESH_NODE_SEND_INT(x)   Dbg_sendInt(x)
#define DBG_MESH_NODE_SEND_HEX(x)   Dbg_sendHex(x)
#define DBG_MESH_NODE_SEND_BYTE(x)  Dbg_sendOneByteHex(x)
#else
#define DBG_MESH_NODE_SEND_STR(x)
#define DBG_MESH_NODE_SEND_INT(x)
#define DBG_MESH_NODE_SEND_HEX(x)
#define DBG_MESH_NODE_SEND_BYTE(x)
#endif
...
#define MD_HSL_HUE_SAT_S_PRIMARY    SIG_MD_LIGHT_HSL_HUE_S,SIG_MD_LIGHT_HSL_SAT_S,
    #endif
    #if MD_SERVER_EN
#define MD_ID_ARRAY_LIGHT_HSL_SERVER    SIG_MD_LIGHT_HSL_S, SIG_MD_LIGHT_HSL_SETUP_S
    #else
#define MD_ID_ARRAY_LIGHT_HSL_SERVER    
    #endif
...
int mesh_cmd_sig_cfg_cps_get(u8 *par, int par_len, mesh_cb_fun_par_t *cb_par)
{
	...
	DBG_MESH_NODE_SEND_STR("\n mesh_cmd_sig_cfg_cps_get");
...
int mesh_provision_par_set(u8 *prov_pars)
{
    ...
    if(prov_par->key_index != 0x0001) {   // not default network
	...
	DBG_MESH_NODE_SEND_STR("\n LGT_CMD_PROV_SUC_EVE");

	 /* Reset node if do not receive add application key
	    message after set node information */
	mesh_set_start_wait_add_appkey_msg_after_set_node_info(TRUE);
...
int app_event_handler_adv(u8 *p_payload, int src_type, u8 need_proxy_and_trans_par_val)
{
	...
	if(adv_type == MESH_ADV_TYPE_MESSAGE){
       ...
       /*
		...
		for(u8 i = 0; i < 50; i++){
			DBG_MESH_NODE_SEND_STR(" ");
			DBG_MESH_NODE_SEND_BYTE(*(p_payload+i));
		}
		*/
#endif
